...
수익화 앱이 아니므로 Supabase는 **Free 플랜으로 운영까지 커버**한다.

- DB 용량(전국 약국 수만 건), Edge Function 호출 수(1시간 1회, 일 24회) 모두 Free 한도 내에서 여유 있음
- Free 플랜의 "7일간 비활성 시 프로젝트 일시정지" 문제는 캐싱 전략의 1시간 주기 cron 자체가 활동으로 잡혀 자연히 회피됨
- 트래픽이 크게 늘어 Free 한도(대역폭 등)를 초과하는 시점이 오면 그때 Pro 플랜 전환을 재검토
